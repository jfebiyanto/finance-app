<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Add Investment') }}</h2>
    </x-slot>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6">
                <form method="POST" action="{{ route('investments.store') }}">
                    ...
                    <div id="term-deposit-fields" class="mb-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200" style="display: none;">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <input type="text" name="bank_name" value="{{ old('bank_name') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g. Bank Mandiri">
                            </div>
                            <div>
                                ...
                                <select name="term_months" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="1" {{ old('term_months') == 1 ? 'selected' : '' }}>1 Month</option>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" max="100" name="interest_rate" value="{{ old('interest_rate') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g. 4.5">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Maturity Date</label>
                                <input type="date" name="maturity_date" value="{{ old('maturity_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div id="shares-fields" class="mb-4 p-4 bg-gray-50 rounded-lg">
                        <h4 class="text-sm font-semibold text-gray-700 mb-3">Shares & Avg Cost (Optional)</h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Shares / Units</label>
                                <input type="number" step="0.0001" min="0" name="shares" value="{{ old('shares') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g. 200">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Buying Price / Avg Cost (Rp)</label>
                                <input type="number" step="0.01" min="0" name="avg_cost" value="{{ old('avg_cost') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="e.g. 580">
                            </div>
                        </div>
                        <p class="text-xs text-gray-400 mt-2">Amount Invested will be auto-calculated as Shares × Avg Cost. You can also fill it manually.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="amount_invested" value="{{ old('amount_invested') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Current Value (Rp)</label>
                            <input type="number" step="0.01" min="0" name="current_value" value="{{ old('current_value') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <p id="current-value-hint" class="text-xs text-gray-400 mt-1">Leave empty to auto-calculate for Term Deposits.</p>
                        </div>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Deposit / Purchase Date</label>
                        <input type="date" name="purchase_date" id="purchase-date" value="{{ old('purchase_date') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                    <div class="mb-4">
